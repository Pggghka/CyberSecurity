package com.vartanian.ssu;

import java.util.Scanner;

public class Program {
    public static void main(String[] args) {
// //       Task1 Cezar
//        String alphabet = "абвгдеёжзийклмнопрстуфхцчшщъыьэюя0123456789!,.?";
//        //String alphabet ="abcdefghijklmnopqrstuvwxyz0123456789!,.?";
//        //n,k - keys
//        Scanner in = new Scanner(System.in);
//        System.out.println("Enter n: "); int n = in.nextInt();
//        System.out.println("Enter k: "); int k = in.nextInt();
//        Task1 task1 = new Task1();
//        System.out.println("Total symbols in the alphabet:" + alphabet.length());
//        if(task1.gcd(n, alphabet.length())==1){
//            task1.Cypher(n, alphabet.length(),k,alphabet,true);
//        }
//        else System.out.println("GCD isn't 1");

//        Task2_3 task23 = new Task2_3();
//        task23.go();
//
//        Task4 polibiaQadr = new Task4();
//        polibiaQadr.polibiaQadr();

//        Task6 Vizner = new Task6();
//        Vizner.go();

//        Task7 cezarWithKey = new Task7();
//        cezarWithKey.go();

//        Task8 paired = new Task8();
//        paired.go();

//        Task9 XOR = new Task9();
//        XOR.go();

        Task6 Transpos = new Task6();
        Transpos.go();


//     Task10 Transpos = new Task10();
//        Transpos.go();
    }
}
